...
inline const Result ModeB(const Config& config) {
    ...
    using GuppiWriter = Pipelines::Generic::Accumulate<Modules::Guppi::Writer<OT>, Device::CPU, OT>;
    std::unique_ptr<Runner<GuppiWriter>> guppiWriterRunner;
    using FilterbankWriter = Pipelines::Generic::Accumulate<Modules::Filterbank::Writer<OT>, Device::CPU, OT>;
    ...
    if constexpr (std::is_same<OT, CF32>::value || std::is_same<OT, CF16>::value) {
        typename GuppiWriter::Config writerConfig = {
            .moduleConfig = {
                ...
                .directio = true,
                .inputFrequencyBatches = 1, // Accumulate pipeline set to reconstituteBatchedDimensions.
            },
